...
/*   Updated: 2020/08/04 19:20:51 by jwon             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			parse_texture(char *line, int id, t_config *config)
{
	...
	if (ft_isspace_str(line))
		exit_program("Texture filename is empty", 0);
	tmp = ft_strtrim(line, " ");
	if (check_filename_ext(tmp, "xpm") == 0)
		exit_program("Texture filename extension is invalid", 0);
...
static void		check_color_format(char *line)
{
	...
	if (comma_cnt != 2 || idx != ((int)ft_strlen(line)))
		exit_program("RGB color format is invalid", 0);
}
...
void			parse_color(char *line, int id, t_config *config)
{
	...
	if (ft_isspace_str(line))
		exit_program("RGB color value is empty", 0);
	tmp = ft_strtrim(line, " ");
	...
	if (!(0 <= color[0] && color[0] <= 255) ||
		...
		exit_program("RGB color is out of expected range", 0);
